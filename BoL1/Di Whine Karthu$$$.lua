...
function OnLoad()
	Menu = scriptConfig("Di Whine Karthu$$$", "bilbao") Menu:addTS(myTs) SxOrb:LoadToMenu(Menu)
		...
		Menu:addParam("bilbao", "$$$  -  i am so di whine  -  $$", SCRIPT_PARAM_LIST, 1, {"rekt", "by", "bilbao"})
end
function OnTick() myTs:update() myMinionHandle:update()
	if myHero:CanUseSpell(_Q) == READY then
		if ((SxOrb.IsFight and Menu.q.IsFight) or (SxOrb.IsHarass and Menu.q.Harass)) and ValidTarget(myTs.target, 800) then
			local predPos, hc = VP:GetCircularCastPosition(myTs.target, 0.75, 175, 800, 1700)
			if hc >= 1 then CastSpell(_Q, predPos.x, predPos.z) end
		elseif ((SxOrb.IsLaneClear and Menu.q.IsLaneClear) or (SxOrb.IsLastHit and Menu.q.IsLastHit)) and ValidTarget(myMinionHandle.objects[1], 800) and ((SxOrb.IsLastHit and myMinionHandle.objects[1].health <  myHero:CalcMagicDamage(myMinionHandle.objects[1], (20 + (myHero:GetSpellData(SPELL_1).level * 20) + (myHero.ap * 0.3)))) or SxOrb.IsLaneClear) then
			local predPos, hc = VP:GetCircularCastPosition(myMinionHandle.objects[1], 0.75, 175, 800, 1700)
			if hc >= 1 then CastSpell(_Q, predPos.x, predPos.z) end
		end
	end
	if myHero:CanUseSpell(_W) == READY then
		if ((SxOrb.IsFight and Menu.w.IsFight) or (SxOrb.IsHarass and Menu.w.Harass)) and ValidTarget(myTs.target, 925) then
	...
	if myHero:CanUseSpell(_E) == READY then
		if ((SxOrb.IsFight and Menu.e.IsFight) or (SxOrb.IsHarass and Menu.e.Harass)) and not EACtive and ValidTarget(myTs.target, 500) then
			CastSpell(_E)
