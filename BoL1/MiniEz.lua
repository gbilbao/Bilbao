...
function OnTick()
	...
	if Menu.fightmode then
		myTs:update()
		if ValidTarget(myTs.target) and CastQOn(myTs.target) then return end
	end
...
function OnDraw()
	if not myHero.dead and Menu.drawq then DrawCircle(myHero.x, myHero.y, myHero.z, 1200,RGB(7, 7, 250)) end
end
